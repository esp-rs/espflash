...
use crate::{
    elf::{
        update_checksum, CodeSegment, FirmwareImage, FlashFrequency, RomSegment, ESP_CHECKSUM_MAGIC,
    },
...
impl<'a> Esp8266Format<'a> {
    pub fn new(image: &'a FirmwareImage) -> Result<Self, Error> {
        ...
        // Common header
        let flash_size = image.flash_size.unwrap_or(FlashSize::Flash4Mb);
        let flash_freq = image.flash_frequency.unwrap_or(FlashFrequency::Flash40M);
