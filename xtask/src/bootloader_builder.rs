...
pub(crate) fn build_bootloaders(workspace: &Path, args: BuildBootloadersArgs) -> Result<()> {
    ...
    for bootloader in selected {
        ...
        let generated = build_bootloader(
            &args.esp_idf_path,
            &idf_tools_path,
            &build_root,
            &manifest.defaults.configs,
            bootloader,
        )?;
...
fn checkout_esp_idf_ref(esp_idf_path: &Path, idf_ref: &str) -> Result<()> {
    ...
    let remote_ref = format!("refs/remotes/origin/{idf_ref}");
    let remote_ref_exists = Command::new("git")
        .arg("-C")
        .arg(esp_idf_path)
        .arg("show-ref")
        .arg("--verify")
        ...
        .status()?
        .success();
    let checkout_ref = if remote_ref_exists {
        remote_ref.as_str()
    } else {
        idf_ref
    };
...
fn build_bootloader(
    ...
    let configs = default_configs
        .iter()
        .chain(&bootloader.configs)
        .map(String::as_str)
        .collect::<Vec<_>>()
        .join("\n");
    fs::write(
        &sdkconfig_defaults,
        format!("# Generated by xtask build-bootloaders\n{configs}"),
    )?;
